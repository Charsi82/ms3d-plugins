#include <msLib.h>
#include "ms3d_ogf_import_test.h"
...
int ms3d_ogf_import_test::import(const xr_bone* xbone, const xr_bone_vec& xbones, int* bone_refs)
{
...
int ms3d_ogf_import_test::import(const xr_ogf *ogf)
{
	int status = 0;
	bool to_CS = false;
	...
	if (ogf->skeletal()) {
		...
		to_CS = (xbones.size() == 45) && (MessageBox(NULL, "Грузим модель НПС из ТЧ. Конвертировать в ЧН/ЗП?", "X-Ray Importer", MB_YESNO) == IDYES);
		if (to_CS){
		...
		//b_pos = { 0, 0.2f, 0 };
		msVec3 pos1 = { 0.000000, 0.980000f, 0.000000 };
		msBone_SetPosition(bone, pos1);
		need_rotate = true;
		};
		
		if ((xbones.size() == 47) &&
			(MessageBox(NULL, "Грузим модель НПС из ЧН/ЗП. Конвертировать в ТЧ?", "X-Ray Importer", MB_YESNO) == IDYES)) {
			
			// экспортировать без первых двух костей
	...
	if (pMdl){
		...
		if (!comment.size())
		{
			...
			{
				std::string bname = (*it)->name();
				comment.append(" ").append(bname).append("\r\n");
			}
		}
		msMesh_SetComment(pMdl, comment.c_str());
	}
	delete[] bone_refs;
	return status;
}
...
int ms3d_ogf_import_test::import(const xr_ogf* ogf, const int* bone_refs)
{
	...
	std::string comment = "gamedata\\textures\\";
	comment.append(ogf->texture());
	msMaterial_SetComment(mtl, comment.append(".dds > ").append(ogf->shader()).c_str());
