...
int ms3d_xray_import::GetType() { return eTypeTool|eNormalsAndTexCoordsPerTriangleVertex; }

const char* ms3d_xray_import::GetTitle() { return "X-Ray NPC Converter (ShoC <-> CS,CoP)"; }

static const TCHAR k_filter[] = _T(
//	"X-Ray objects (*.ogf;*.dm;*.object)\0*.ogf;*.dm;*.object\0"
	"X-Ray objects (*.ogf)\0*.ogf\0"
...
int ms3d_xray_import::Execute(msModel* model)
{
	...
	if (msModel_GetMeshCount(model) || msModel_GetMaterialCount(model) || msModel_GetBoneCount(model))
	{
		MessageBox(NULL, "Перед импортом конвертируемой модели следует очистить сцену.", "X-Ray NPC Converter", MB_OK | MB_ICONWARNING);
		return -1;
	}
