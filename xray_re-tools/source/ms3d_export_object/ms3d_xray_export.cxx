...
//int ms3d_xray_export::GetType() { return eTypeExport|eNormalsAndTexCoordsPerTriangleVertex; }// can't export with flag eNormals...
int ms3d_xray_export::GetType() { return eTypeExport; }

const char* ms3d_xray_export::GetTitle() { return "S.T.A.L.K.E.R... (*.object)"; }
...
//	ofn.nMaxFileTitle = MAX_PATH;//+
	xr_file_system& fs = xr_file_system::instance();
	ofn.lpstrInitialDir = fs.resolve_path(PA_OBJECTS);
//	ofn.lpstrTitle = NULL;
...
//		return -1;
	...
	while (true)
	{
		...
		if ((strlen(path) < strlen(ext) + 1) || (strcmp(path + strlen(path) - strlen(ext), ext) != 0))
			strcat_s(path, ext);

		if (!fs.file_exist(path) ||
				IDYES == MessageBox(NULL,
				"Файл с таким именем уже существует. Перезаписать?",
				"Подтверждение перезаписи файла", MB_YESNO | MB_ICONWARNING))
			break;
	}
